...
describe('Jetton', () => {
    ...
    beforeEach(async () => {
        ...
        let jettonRaw = await SampleJetton.init(deployer.address, content, max_supply);
        jetton = blockchain.openContract(
            await SampleJetton.fromInit(deployer.address, content, max_supply),
        );

        let deployAmount = toNano('0.15');

        let supply = toNano(100000000); // 🔴 Specify total supply in nano
      ...
      //   expect(deploy1Result.transactions).toHaveTransaction({
      //     // from: tester.address,
      //     to: jetton.address,
      //     deploy: true,
      //     success: true,
    ...
    it('should deploy', async () => {
        ...
        deployerWallet = blockchain.openContract(
            await JettonDefaultWallet.fromInit(deployer.address, jetton.address),
        );
